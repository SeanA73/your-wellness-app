...
export default function AdminDashboard() {
  ...
  // Deliberately inert. The origin/main version of this handler upserted a
  // `subscriptions` row and wrote `profiles.subscription_plan` straight from the
  // browser, which is the paywall bypass the cleanup branch revoked: entitlement
  // is owned by the Stripe webhook using the service role key. The applied schema
  // now backs that up — `subscriptions` is SELECT-only for clients and
  // `profiles.subscription_plan` is excluded from the column-level UPDATE grant,
  // so these writes would fail at the database anyway.
  //
  // Kept as a stub (rather than deleted) so the control below still type-checks
  // while a server-side replacement is decided on.
  const handleUpdateSubscription = async (_userId: string, _status: string) => {
    toast({
      title: "Not available",
      description:
        "Subscription changes must go through Stripe. Granting premium from the admin UI is disabled.",
      variant: "destructive"
    });
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            {/* "Create Test Users" removed with the create-test-users edge
                function: it seeded two admin accounts with a hardcoded
                password. Seed test data with a local script, not a shipped
                endpoint. */}
          </div>
        </div>
